...
export const contractService = {
  async renewByLessor(lessorId: string, contractId: string, input: { startDate: Date; endDate?: Date | null; rentAmount?: number }) {
    const contract = await prisma.contract.findUnique({
      where: { id: contractId },
    });
    if (!contract) throw new AppError('Contract not found', 404);
    if (contract.lessorId !== lessorId) throw new AppError('Unauthorized', 403);
    ...
    return prisma.contract.update({
      ...
      data: {
        ...
        rentAmount: input.rentAmount ?? contract.rentAmount,
        status: ContractStatus.ACTIVE, // Re-activate if it was ended
      },
      select: contractWithRelations,
    });
  },

  async listActiveForUser(userId: string, role: UserRole) {
    const where =
      role === UserRole.LESSOR
        ? { lessorId: userId, status: ContractStatus.ACTIVE }
        : { lesseeId: userId, status: ContractStatus.ACTIVE };
  ...
  async archiveByLessor(lessorId: string, contractId: string) {
    const contract = await prisma.contract.findUnique({
      where: { id: contractId },
    });
    if (!contract) throw new AppError('Contract not found', 404);
    if (contract.lessorId !== lessorId) throw new AppError('Unauthorized', 403);
    if (contract.status !== ContractStatus.ENDED) {
      throw new AppError('Only ended contracts can be archived', 400);
    }
